...
CFG = yaml.safe_load(open("datasets/semantic_kitti/semantic-kitti.yaml", 'r'))
...
def convert_labels_to_vis_colors(seg):
    """

    :param seg: np array
    :return:
    """
    seg_vis = SEMANTIC_KITTI_COLORMAP[np.array([CFG['learning_map_inv'][l] for l in seg])]
    return seg_vis
...
def convert_coords_to_grid_pts_batch(pts, coords, ds):
    a = pts.max(dim=1)[0] - 0.5 * ds
    b = coords.max(dim=1)[0]
    c = pts.min(dim=1)[0] + 0.5 * ds
    d = coords.min(dim=1)[0]
    alpha = (a - c) / (b - d)
    beta = (b * c - a * d) / (b - d)
    # grid_pts = coords * alpha + beta
    return alpha, beta
